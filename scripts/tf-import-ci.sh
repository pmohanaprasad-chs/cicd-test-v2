...
if aws ecr describe-repositories --repository-names "$PROJECT" > /dev/null 2>&1; then
  ...
  # Import lifecycle policy if it exists
  aws ecr get-lifecycle-policy --repository-name "$PROJECT" > /dev/null 2>&1 && \
    tf_import "module.ecr.aws_ecr_lifecycle_policy.main" "$PROJECT" || true
fi

# ── IAM ──────────────────────────────────────────────────────────────────────
# ECS task execution role
aws iam get-role --role-name "${PROJECT}-ecs-task-execution" > /dev/null 2>&1 && \
  tf_import "module.ecs.aws_iam_role.task_execution" "${PROJECT}-ecs-task-execution" || true

# ECS instance role
aws iam get-role --role-name "${PROJECT}-ecs-instance" > /dev/null 2>&1 && \
  tf_import "module.ecs.aws_iam_role.ecs_instance" "${PROJECT}-ecs-instance" || true

# ECS instance profile
aws iam get-instance-profile --instance-profile-name "${PROJECT}-ecs-instance-profile" > /dev/null 2>&1 && \
  tf_import "module.ecs.aws_iam_instance_profile.ecs_instance" "${PROJECT}-ecs-instance-profile" || true

# ECS role policy attachments
aws iam get-role --role-name "${PROJECT}-ecs-task-execution" > /dev/null 2>&1 && \
  tf_import "module.ecs.aws_iam_role_policy_attachment.task_execution" \
    "${PROJECT}-ecs-task-execution/arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy" || true

aws iam get-role --role-name "${PROJECT}-ecs-instance" > /dev/null 2>&1 && \
  tf_import "module.ecs.aws_iam_role_policy_attachment.ecs_instance" \
    "${PROJECT}-ecs-instance/arn:aws:iam::aws:policy/service-role/AmazonEC2ContainerServiceforEC2Role" || true

# GitHub Actions IAM role + OIDC (always exist — never destroyed)
aws iam get-role --role-name "${PROJECT}-github-actions" > /dev/null 2>&1 && \
  tf_import "module.iam.aws_iam_role.github_actions" "${PROJECT}-github-actions" || true
...
for ENV in dev staging prod; do
  aws logs describe-log-groups \
    ...
    --query 'logGroups[0].logGroupName' --output text > /dev/null 2>&1 && \
    tf_import "module.ecs.aws_cloudwatch_log_group.ecs[\"${ENV}\"]" "/ecs/${PROJECT}/${ENV}" || true
done
...
if [[ -n "$VPC_ID" && "$VPC_ID" != "None" ]]; then
  ...
  [[ -n "$IGW_ID" && "$IGW_ID" != "None" ]] && \
    tf_import "module.networking.aws_internet_gateway.main" "$IGW_ID" || true
  ...
  [[ -n "$PUB_RT" && "$PUB_RT" != "None" ]] && \
    tf_import "module.networking.aws_route_table.public" "$PUB_RT" || true
  ...
  [[ -n "$PRIV_RT" && "$PRIV_RT" != "None" ]] && \
    tf_import "module.networking.aws_route_table.private" "$PRIV_RT" || true

  NAT_ID=$(aws ec2 describe-nat-gateways \
    --filter "Name=vpc-id,Values=${VPC_ID}" "Name=state,Values=available" \
    --query 'NatGateways[0].NatGatewayId' --output text 2>/dev/null || echo "")
  [[ -n "$NAT_ID" && "$NAT_ID" != "None" ]] && \
    tf_import "module.networking.aws_nat_gateway.main[0]" "$NAT_ID" || true
  ...
  [[ -n "$EIP_ALLOC" && "$EIP_ALLOC" != "None" ]] && \
    tf_import "module.networking.aws_eip.nat[0]" "$EIP_ALLOC" || true
...
[[ -n "$ALB_SG" && "$ALB_SG" != "None" ]] && \
  tf_import "module.ecs.aws_security_group.alb" "$ALB_SG" || true

ECS_SG=$(aws ec2 describe-security-groups \
  --filters "Name=group-name,Values=${PROJECT}-ecs-sg" \
  --query 'SecurityGroups[0].GroupId' --output text 2>/dev/null || echo "")
[[ -n "$ECS_SG" && "$ECS_SG" != "None" ]] && \
  tf_import "module.ecs.aws_security_group.ecs_instances" "$ECS_SG" || true
...
if [[ -n "$ALB_ARN" && "$ALB_ARN" != "None" ]]; then
  ...
  for ENV in dev staging prod; do
    ...
    [[ -n "$TG_ARN" && "$TG_ARN" != "None" ]] && \
      tf_import "module.ecs.aws_lb_target_group.env[\"${ENV}\"]" "$TG_ARN" || true
  done
  ...
  if [[ -n "$LISTENER_ARN" && "$LISTENER_ARN" != "None" ]]; then
    ...
    [[ -n "$DEV_RULE" && "$DEV_RULE" != "None" ]] && \
      tf_import "module.ecs.aws_lb_listener_rule.dev" "$DEV_RULE" || true
    [[ -n "$STAGING_RULE" && "$STAGING_RULE" != "None" ]] && \
      tf_import "module.ecs.aws_lb_listener_rule.staging" "$STAGING_RULE" || true
  fi
fi
...
[[ -n "$CP_ARN" && "$CP_ARN" != "None" ]] && \
  tf_import "module.ecs.aws_ecs_capacity_provider.main" "${PROJECT}-cp" || true
...
ASG=$(aws autoscaling describe-auto-scaling-groups \
  ...
  --output text 2>/dev/null || echo "")
[[ -n "$ASG" && "$ASG" != "None" ]] && \
  tf_import "module.ecs.aws_autoscaling_group.ecs" "$ASG" || true
...
LT_ID=$(aws ec2 describe-launch-templates \
  ...
  --output text 2>/dev/null || echo "")
[[ -n "$LT_ID" && "$LT_ID" != "None" ]] && \
  tf_import "module.ecs.aws_launch_template.ecs" "$LT_ID" || true
...
for ENV in dev staging prod; do
  SVC=$(aws ecs describe-services \
    ...
    --output text 2>/dev/null || echo "")
  [[ -n "$SVC" && "$SVC" != "None" ]] && \
    tf_import "module.ecs.aws_ecs_service.${ENV}" "${CLUSTER}/${PROJECT}-${ENV}" || true
  ...
  TD=$(aws ecs describe-task-definition \
    ...
    --output text 2>/dev/null || echo "")
  [[ -n "$TD" && "$TD" != "None" ]] && \
    tf_import "module.ecs.aws_ecs_task_definition.env[\"${ENV}\"]" "$TD" || true
done
